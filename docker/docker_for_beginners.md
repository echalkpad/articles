...
[Original URL](http://prakhar.me/docker-curriculum/)
...
[![Fork me on GitHub](https://s3.amazonaws.com/github/ribbons/forkme_right_darkblue_121621.png)](https://github.com/prakhar1989/docker-curriculum) []() ![docker logo](https://raw.githubusercontent.com/prakhar1989/docker-curriculum/master/images/logo.png)

_Learn to build and deploy your distributed applications easily to the cloud with Docker_

Written and developed by [Prakhar Srivastav](http://prakhar.me).

[Top](http://prakhar.me/docker-curriculum/#top)
...
**Google Trends for 'Docker'** ![](https://raw.githubusercontent.com/prakhar1989/docker-curriculum/master/images/interest.png "interest")
...
[Top](http://prakhar.me/docker-curriculum/#table-of-contents)
...
Until a few releases ago, running Docker on OSX and Windows was quite a hassle. Lately however, Docker has invested significantly into improving the on-boarding experience for its users on these OSes, thus running Docker now is a cakewalk. The _getting started_ guide on Docker has detailed instructions for setting up Docker on [Mac](https://www.docker.com/products/docker#/mac), [Linux](https://www.docker.com/products/docker#/linux) and [Windows](https://www.docker.com/products/docker#/windows).
...
- _Docker Daemon_ - The background service running on the host that manages building, running and distributing Docker containers. The daemon is the process that runs in the operation system to which clients talk to.
...
![](https://raw.githubusercontent.com/prakhar1989/docker-curriculum/master/images/static.png "static")
...
$ python app.py
 aria2cweb.service articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js mailpile MITIE netdata png pngquant start-thesrc thesrc traefik.sample.toml ucii Running on http://0.0.0.0:5000/ (Press CTRL+C to quit)
```
...
![](https://raw.githubusercontent.com/prakhar1989/docker-curriculum/master/images/catgif.png "static")
...
![](http://prakhar.me/docker-curriculum/images/eb-start.png "static")
...
- The following screen is shown below. Choose _Docker_ from the predefined configuration. You can leave the _Environment type_ as it is. Click Next.

![](http://prakhar.me/docker-curriculum/images/eb-docker.png "static")
...
- For now, we won't be making changes in the _Additional Resources_ section. Click Next and move to _Configuration Details_.
...
- We also don't need to provide any _Environment Tags_ and _Permissions_, so without batting an eyelid, you can click Next twice in succession. At the end, the screen shows us the _Review_ page. If everything looks good, go ahead and press the **Launch** button.
...
![](http://prakhar.me/docker-curriculum/images/eb-deploy.png "static")
...
![sf food trucks](https://raw.githubusercontent.com/prakhar1989/FoodTrucks/master/shot.png)
...
2. Since the _bridge_ network is shared by every container by default, this method is **not secure**.
...
The `network create` command creates a new _bridge_ network, which is what we need at the moment. There are other [kinds](https://docs.docker.com/engine/userguide/networking/dockernetworks/) of networks that you can create, and you are encouraged to read about them in the official docs.
...
Head over to <http://0.0.0.0:5000> and see your glorious app live! Although that might have seemed like a lot of work, we actually just typed 4 commands to go from zero to running. I've collated the commands in a [bash script](https://github.com/prakhar1989/FoodTrucks/blob/master/setup-docker.sh).
...
So what is _Compose_ used for? Compose is a tool that is used for defining and running multi-container Docker apps in an easy way. It provides a configuration file called `docker-compose.yml` that can be used to bring up an application and the suite of services it depends on with just one command.
...
web_1 | aria2cweb.service articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js mailpile MITIE netdata png pngquant start-thesrc thesrc traefik.sample.toml ucii Running on http://0.0.0.0:5000/ (Press CTRL+C to quit)
```
...
While we're are at it, we'll also remove the `foodtrucks` network that we created last time. This should not be required since _Compose_ would automatically manage this for us.
...
Great! Now that we have a clean slate, let's re-run our services and see if _Compose_ does it's magic.
...
```
[email protected]:/opt/flask-app# ping es
...
Voila! That works. So somehow, this container is magically able to ping `es` hostname. It turns
out that in Docker 1.10 a new networking system was added that does service discovery using a DNS server. If you're interested, you can read more about the [proposal](https://github.com/docker/libnetwork/issues/767) and [release notes](https://blog.docker.com/2016/02/docker-1-10/).
...
![keypair.png](http://prakhar.me/docker-curriculum/images/keypair.png)
...
Go ahead and open <http://54.86.14.14> in your browser and you should see the Food Trucks in all its black-yellow glory! Since we're on the topic, let's see how our [AWS ECS](https://console.aws.amazon.com/ecs/home?region=us-east-1#/clusters) console looks.

![ECS cluster](http://prakhar.me/docker-curriculum/images/cluster.png) ![ECS cluster](http://prakhar.me/docker-curriculum/images/tasks.png)
...
Send in your thoughts directly to [me](http://prakhar.me/cdn-cgi/l/email-protection#2c5c5e4d47444d5e6c5c5e4d47444d5e024149) or just [create an issue](https://github.com/prakhar1989/docker-curriculum/issues/new). I'm on [Twitter](https://twitter.com/prakharsriv9), too, so if that's your deal, feel free to holler there!
