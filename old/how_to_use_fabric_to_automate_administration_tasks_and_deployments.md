...
Local helpers such as the `lcd` context manager (which is used for setting the **local** working directory) are _honoured_ with `local`, the same way `run` (or `sudo`) honours the `cd` context manager.
