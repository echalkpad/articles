...
![](https://iamtrask.github.io/img/neural_stack/neural_stack_picture.png) **What I'm Thinking When I See This:** Ok... so we can push green vectors v_t onto the stack. Each yellow bubble on the right of each green v_t looks like it coresponds to the weight of v_t in the stack... which can be 0 apparently (according to the far right bubble). So, even though there isn't a legend, I suspect that the yellow circles are infact s_t. This is very useful. Furthermore, it looks like the graphs go from left to right. So, this is 3 timesteps. t=1, t=2, and t=3\. Great. I think I see what's going on.
...
![](https://iamtrask.github.io/img/neural_stack/neural_stack_formulas_14.png) If the previous circle returned 1 - the sum of the weights above a layer in the stack, then it guarantees that the weight is 0 if the layer is too deep. However, if the vector is much shallower than 1.0, the previous circle would return a very positive number (perhaps as high as 1 for the vector at the top). This min function guarantees that the weight at this level doesn't exceed the strength that was pushed onto this level s_t[i].
...
# layer['r_t-1'][0] = 1
 ...
 layer['xo_t_delta'] = layer['xo_t_error'] articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii sigmoid_out2deriv(layer['xo_t']) 

 layer['x_o_prime_x_t_error'] = (layers[i-1]['x'] - layer['o_prime_x_t'])
 layer['x_o_prime_x_t_delta'] = layer['x_o_prime_x_t_error'] articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii sigmoid_out2deriv(layer['o_prime_x_t'])
 else:
...
# print row
 ...
 layer['o_t_delta'] = layer['o_t_error'] articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii sigmoid_out2deriv(layer['o_t'])
 ...
 layer = layers[i]
 if(it<2000):
 max_alpha = 0.05 articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii batch_size
# else:
# max_alpha = 0.05 articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii batch_size
 alpha = max_alpha / sub_sequence_length
 ...
 W_hh_update += alpha articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii np.outer(layer['h_t-1'],layer['h_t_delta'])
 W_rh_update += alpha articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii np.outer(layer['r_t-1'],layer['h_t_delta'])
 W_hox_update += alpha articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii np.outer(layer['h_t'],layer['xo_t_delta'])
 ...
 W_opx_update += alpha articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii np.outer(layer['o_prime_t'],layer['x_o_prime_x_t_delta'])
 ...
 b_d_update += alpha articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii layer['d_t_delta']# articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii 10
 b_u_update += alpha articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii layer['u_t_delta']# articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii 10
 b_v_update += alpha articles _articles bin CAs devops Documents dotfiles gethtml go hnews jason js netdata start-thesrc thesrc ucii layer['v_t_delta']
...
**6:** Not sure how many epochs this was trained on. I presume it was many given DeepMind's access to GPUs.
